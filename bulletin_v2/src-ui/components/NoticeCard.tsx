...
import type { Notice, UserProfile, UserSession } from '../types'
...
function Avatar({ profile, size = 40 }: { profile: UserProfile | null; size?: number }){
...
export function NoticeCard({ notice, session, get_profile, on_reply, on_edit, on_vote }: NoticeCardProps){
  const is_mine = session?.userId === notice.userId
  const my_vote = notice.votes.find((v) => v.userId === session?.userId)?.vote ?? null
  ...
  const my_profile = session?.userId ? get_profile(session.userId) : null
  ...
  function open_reply(): void { set_replying(true); set_leaving(false) }
        ...
        <div className="card-body">
          ...
          {replying && (
            <div
              ...
              onAnimationEnd={() => { if (leaving) { set_replying(false); set_leaving(false) } }}
            >
