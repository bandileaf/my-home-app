...
import type { Schedule, UserProfile, UserSession } from '../types'
import type { useSchedules } from '../hooks/useSchedules'
import { display_name_of, initials_of } from '../utils'

type SchedulesHook = ReturnType<typeof useSchedules>
...
function parse_hm(t: string): [number, number] { const p = t.split(':'); return [parseInt(p[0] ?? '9') || 0, parseInt(p[1] ?? '0') || 0] }
function add_hour(t: string): string { const [h, m] = parse_hm(t); return `${pad2((h + 1) % 24)}:${pad2(m)}` }
...
function TimeInput({ value, onChange, disabled }: { value: string; onChange: (v: string) => void; disabled?: boolean }) {
  ...
  return (
    <div className={`cal-tp${disabled ? ' cal-tp-off' : ''}`}>
      <button type="button" className="cal-tp-ampm" onClick={() => { if (!disabled) onChange(hm_to_str(is_pm ? h24 - 12 : h24 + 12, m)) }} disabled={disabled}>
        {is_pm ? '오후' : '오전'}
      </button>
      <input className="cal-tp-h" type="number" min={1} max={12} value={h12} disabled={disabled}
        onChange={e => { const v = Math.max(1, Math.min(12, parseInt(e.target.value) || 1)); onChange(hm_to_str(is_pm ? (v === 12 ? 12 : v + 12) : (v === 12 ? 0 : v), m)) }} />
      <span className="cal-tp-colon">:</span>
      <input className="cal-tp-m" type="number" min={0} max={59} value={pad2(m)} disabled={disabled}
        onChange={e => onChange(hm_to_str(h24, Math.max(0, Math.min(59, parseInt(e.target.value) || 0))))} />
    </div>
  )
}

function is_active(s: Schedule, dateStr: string): boolean {
  if (s.repeatWeekly) return new Date(s.date + 'T00:00:00').getDay() === new Date(dateStr + 'T00:00:00').getDay() && dateStr >= s.date
  if (s.repeatMonthly) return new Date(s.date + 'T00:00:00').getDate() === new Date(dateStr + 'T00:00:00').getDate() && dateStr >= s.date
...
function EvtAvatar({ profile }: { profile: UserProfile | null }) {
  const sz = 18
...
export function CalendarPage({ session, hook, get_profile }: CalendarPageProps) {
  ...
  function prev_month(): void { if (month === 0) { set_year(y => y - 1); set_month(11) } else set_month(m => m - 1) }
  function next_month(): void { if (month === 11) { set_year(y => y + 1); set_month(0)  } else set_month(m => m + 1) }
  ...
  async function handle_submit(): Promise<void> {
    if (!f_title.trim() || !session?.userId) return
    await hook.create({
      title: f_title.trim(), date: f_date,
      endDate: f_edate >= f_date ? f_edate : null,
      allDay: f_allday,
      startTime: f_allday ? null : f_time,
      endTime: f_allday ? null : f_etime,
      repeatWeekly: f_weekly, repeatMonthly: f_monthly,
      memo: f_memo.trim() || null, color: f_color,
    })
    set_composing(false)
  }

  const sel_events = useMemo(() =>
    hook.schedules.filter(s => is_active(s, selected)).sort((a, b) => {
      if (a.allDay !== b.allDay) return a.allDay ? -1 : 1
      return (a.startTime ?? '') < (b.startTime ?? '') ? -1 : 1
    }), [hook.schedules, selected])
            ...
            week.map((dateStr, di) => {
              ...
              return (
                ...
                >
                  {dateStr && (
                    <>
                      <span className="cal-day-num">{parseInt(dateStr.split('-')[2] ?? '0')}</span>
          ...
          : (
            <div className="cal-events-list">
              {sel_events.map(ev => {
                ...
                const is_mine = ev.userId === session?.userId
                return (
                  <div key={ev.id} className="cal-event-item">
                    ...
                    {is_mine && (
                      <button className="cal-event-del" onClick={() => void hook.remove(ev.id)}>
                        <Trash2 size={13} />
                      </button>
                    )}
                  </div>
        ...
        <div className="cal-compose" style={{ left: compose_pos.x, top: compose_pos.y, width: PANEL_W }}>
          ...
          <div className="cal-compose-body">
            ...
            <div className="cal-dt-row">
              ...
              <TimeInput value={f_time}  onChange={v => { set_f_time(v); set_f_etime(add_hour(v)) }} disabled={f_allday} />
            ...
            <textarea className="cal-field cal-memo" placeholder="메모 (선택)" value={f_memo} onChange={e => set_f_memo(e.target.value)} />
          </div>
