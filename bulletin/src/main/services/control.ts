...
function relaunch(ctx: ControlContext, reason: string): void {
  const execPath = process.env.PORTABLE_EXECUTABLE_FILE ?? undefined
  ctx.log(`control: relaunch reason=${reason} execPath=${execPath ?? '(default)'}`)
  app.relaunch({ ...(execPath ? { execPath } : {}), args: process.argv.slice(1).concat([`--post-restart`, `--reason=${reason}`]) })
}
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (method === 'POST' && url === '/restart') {
        ...
        setTimeout(() => { relaunch(ctx, 'remote-restart'); app.quit() }, 500)
        return
      }
